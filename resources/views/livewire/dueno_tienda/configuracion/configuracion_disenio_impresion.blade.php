<div x-data="disenioImpresion">
    ...
    <flux:heading>Configuración de Diseños de Impresión</flux:heading>
    <flux:subheading>Selecciona el diseño predeterminado por negocio y sucursal</flux:subheading>
            ...
            <div class="space-y-3">
                @foreach ($negocios as $negocio)
                    <button type="button" wire:click="seleccionarNegocio({{ $negocio->id }})"
                        class="w-full flex items-center justify-between h-auto p-4 rounded-md border border-border {{ $selectedNegocio == $negocio->id ? 'bg-muted' : 'bg-card hover:bg-muted' }}">
                        <div class="text-left">
            ...
            <div class="space-y-3">
                @forelse($sucursales as $sucursal)
                    <button type="button" wire:click="$set('selectedSucursal', {{ $sucursal->id }})"
                        class="w-full flex items-center justify-between h-auto p-4 rounded-md border border-border 
                                                                                                                                       {{ $selectedSucursal == $sucursal->id
                                                                                                                                           ? 'bg-muted' : 'bg-card hover:bg-muted' }}">
                        <div class="text-left">
            ...
            <div class="space-y-3">
                @if ($selectedSucursal)
                    @foreach ($tiposComprobante as $tipo)
                        <button type="button" wire:click="seleccionarTipoComprobante('{{ $tipo->codigo }}')"
                            class="w-full flex items-center justify-between h-auto p-4 rounded-md border border-border 
    ...
    @if ($selectedTipoComprobante)
        <x-card class="mt-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div wire:click="seleccionarDiseno('default')"
                ...
                @foreach ($disenosDisponibles as $diseno)
                    <div wire:click="seleccionarDiseno({{ $diseno->id }})"
                        class="cursor-pointer rounded-lg border p-4 transition-all
                        ...
                        <div
                            class="aspect-[4/5] bg-muted rounded-md mb-3 overflow-hidden flex items-center justify-center">
                        ...
                        @if ($selectedDiseno === $diseno->id)
                            <div class="mt-2 flex items-center justify-center">
                                <span
                                    class="px-2 py-1 text-xs rounded bg-indigo-500 text-white flex items-center gap-1">
                                    <i class="fa fa-check"></i> Seleccionado
                                </span>
                            </div>
                        @endif
    ...
    @if ($selectedTipoComprobante && $selectedDiseno != 'default')
        <x-card class="mt-6 p-6 space-y-4">
            ...
            <x-flex class="pt-4 justify-end">
                <flux:button wire:click="guardarConfig()">
