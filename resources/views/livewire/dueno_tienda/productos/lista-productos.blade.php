<div>
    ...
    <x-table>
        <x-slot name="thead">
            ...
            <x-table.th>Precio Base</x-table.th>
        ...
        <x-slot name="tbody">
            @forelse($productos as $producto)
                <x-table.tr>
                    ...
                    <x-table.td>
                        @if ($producto->imagen_path)
                            <img src="{{ Storage::disk('public')->url($producto->imagen_path) }}"
                                alt="{{ $producto->nombre_producto }}" class="h-10 w-10 object-cover rounded">
                        @else
                            <x-imagen-empty/>
                        @endif
                    </x-table.td>
                    <x-table.td>{{ $producto->nombre_producto }}</x-table.td>
                    ...
                    <x-table.td>{{ number_format($producto->precio_base, 2) }}</x-table.td>
                    <x-table.td>
                        <span
                    ...
                    <x-table.td>
                        <div class="flex space-x-2">
                            ...
                            <flux:button variant="danger" wire:click="delete('{{$producto->uuid}}')" wire:confirm="¿Está seguro de eliminar este registro?"
                                icon="trash" size="sm">
                                Eliminar
                            </flux:button>
                        </div>
