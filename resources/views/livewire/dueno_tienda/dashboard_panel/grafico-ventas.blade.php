<x-card>
    <div class="flex items-center justify-between mb-4">
        <div>
            ...
            <x-card-text class=" mt-1">
                Comparación diaria de ingresos
            </x-card-text>
        </div>
        @if (isset($ventas_semanales['crecimiento']))
            <span class="text-xs bg-blue-100 dark:text-gray-900 text-blue-700 px-2 py-1 rounded-full inline-flex items-center gap-1">
        ...
        @if (isset($ventas_semanales['datos']))
            @foreach ($ventas_semanales['datos'] as $ventas_semanal)
                ...
                <div class="flex items-center gap-4">
                    <div class="w-8 text-sm font-medium text-gray-600 dark:text-blue-200">
                        {{ $ventas_semanal['day'] }}
                    </div>
